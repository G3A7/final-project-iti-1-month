...
const Products = ({ state, statePage, setTotalPages, search }) => {
  ...
  useEffect(() => {
    search.length > 0
      ? fetch(
          `https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&query=${search}&include_adult=false&include_video=false&language=en-US&page=${statePage}&sort_by=popularity.desc${
      ...
      : state == 1
      ?  fetch( `https://api.themoviedb.org/3/movie/top_rated?api_key=${API_KEY}&language=en-US&page=${statePage}`)
          .then((res) => res.json())
          .then((data) => {
            setTotalPages(data.total_pages);
            getData(data.results);
